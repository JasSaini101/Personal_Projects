''''Learned how to add audio files into the game. Added three files into the directory but just like I did with images,
I put them all in a folder so when I wanted to call these files, I would need to state the path in the directory to get
to them. They were all mp3 files however .wav works in pygame too (but only for soundeffects). The sound effects are for
when a bullet is shot and when the goblin gets hit. There is also background music that constantly plays. Along with this,
also made it so that when the player collides with the goblin, he is reset on the screen and a giant -5 appears that indicates
points were taken away from the score. Thus, the player can no longer run through the goblin.'''
...
pygame.display.set_caption("Sound Effects and Music") 
